...
export function ComposeBlockDialog({
  ...
  useEffect(() => {
    ...
    if (cat?.workspace && cat.workspace.templates.length > 0) {
      ...
      if (!current) {
        setSelectedTemplate(cat.workspace.templates[0].name);
      }
    } else {
      setSelectedTemplate(null);
    }
  }, [kind, data.categories, selectedTemplate]);
  ...
  function submit(e: React.FormEvent) {
    ...
    if (mode === "routine") {
      ...
      const error = addRoutine({ day: Number(day), start, end, endsNextDay, kind, title: title.trim(), notes: serializeNotes(noteLines), workspace: wsData });
      if (error) {
      ...
      const error = addCommitment({ date: commitmentDate, start, end, endDate: normalizedEndDate, endsNextDay: effectiveEndsNextDay, kind, title: title.trim(), notes: serializeNotes(noteLines), priority, workspace: wsData });
      if (error) {
        toast({ title: "Scheduling conflict", description: error });
        return;
      }
      if (saveAsPreset) {
        addPreset({ title: title.trim(), titleCustom: undefined, kind, duration: hasDate ? durationMin(start, end) : looseDuration, notes: serializeNotes(noteLines), priority: { urgent: commitmentUrgent, important: commitmentImportant }, workspace: wsData });
      }
      toast({
      ...
      <DialogTrigger asChild>
        ...
        <DialogHeader>
          ...
          {(() => {
            ...
            return (
              <div className="mt-2">
                <Label className="text-[10px] uppercase tracking-wider text-muted-foreground">{localePt ? "Programa" : "Program"}</Label>
