...
// Small 30×42 hourglass — simplified version of the Focus page one.
...
export function TimerCard() {
  ...
  return (
    <div className={`w-full rounded-lg border px-3 py-2.5 flex items-center gap-2.5 transition-colors ${
      hasActivity ? "border-secondary/25 bg-secondary/5" : "border-sidebar-border/40 bg-transparent"
    }`}>
      {/* Status — click to open Focus */}
      <button
      ...
      >
        <div className="shrink-0 relative">
          {hasActivity ? (
            <>
              <MiniHourglass ratio={ratio} size={30} />
              {timer.running && (
                <span className="absolute -top-0.5 -right-0.5 h-2 w-2 rounded-full bg-secondary animate-pulse ring-2 ring-sidebar" />
              )}
            </>
          ) : (
            <div className="h-[42px] w-[30px] flex items-center justify-center">
              <Hourglass className="h-5 w-5 text-sidebar-foreground/20 group-hover/timer:text-sidebar-foreground/35 transition-colors" />
            </div>
          )}
        </div>
        ...
        <div className="flex-1 min-w-0">
          {isIdle ? (
            <div className="text-[11px] text-sidebar-foreground/35 italic group-hover/timer:text-sidebar-foreground/50 transition-colors">
              No timer active
            </div>
          ) : (
            <>
      ...
      {/* Inline controls when a timer is active */}
      {hasActivity && (
