...
export function DigestView({ digest, onSendToChat }: { digest: Digest; onSendToChat?: (prompt: string) => void }) {
  return (
    <div className="space-y-4">
      <div className="chronos-card rounded-lg border border-border/60 p-3">
        <div className="flex items-center gap-2 mb-2">
          <CalendarDays className="h-4 w-4 text-secondary" />
          <span className="font-display text-[13px] text-primary">{TIMEFRAME_LABELS[digest.timeframe]}</span>
          <span className="text-[9px] tabular-nums px-1.5 py-0.5 rounded-full border border-secondary/30 bg-secondary/5 text-secondary">
            {digest.date}
          </span>
      ...
      <div className="space-y-2">
        {digest.cards.map((card, i) => (
          <ReportCardView key={`${card.kind}-${i}`} card={card} />
        ))}
      </div>
